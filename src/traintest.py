...
parser.add_argument("--exp-dir", type=str, default="/ssd_scratch/cvit/vanshg/sal_project/exp", help="directory to dump experiments")
...
parser.add_argument("--model", type=str, default='gopt_nophn', help="name of the model")
...
def train(audio_model, train_loader, test_loader, args):
    ...
    while epoch < args.n_epochs:
        ...
        log_dict = {
            ...
            'test_phone_mse': te_mse.item(),
            'test_phone_corr': te_corr,
            'test_utt_acc': te_utt_corr[0],
            'test_utt_com': te_utt_corr[1],
            'test_utt_flu': te_utt_corr[2],
            'test_utt_proc': te_utt_corr[3],
            'test_utt_total': te_utt_corr[4],
            'test_word_acc': te_word_corr[0],
            'test_word_stress': te_word_corr[1],
            'test_word_total': te_word_corr[2]
        }
        wandb.log(log_dict)
        print(log_dict)
...
def main():
    ...
    project_name = 'sal_project'
    run_name = f"{args.model}_emb{args.embed_dim}_depth{args.goptdepth}_heads{args.goptheads}"
    args.exp_dir = os.path.join(args.exp_dir, run_name)
